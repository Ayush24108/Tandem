...
const PATTERNS: Record<PatternType, (x: number, y: number) => number | null> = {
  random: () => null,
...
const buildGrid = ({ width, height, pixelSize, gap, pattern, randomness }: GridParams) => {
  let size = pixelSize;
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }: KeyframeParams) => {
  const windowKeyframes: Keyframe[] = [];
  ...
  return { window: windowKeyframes, content: contentKeyframes };
};
...
export default function PixelSwap({
  ...
  const [transition, setTransition] = useState<{ to: boolean; grid: ReturnType<typeof buildGrid> } | null>(null);
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      const timing: KeyframeEffectOptions = { duration: pixelMs, delay: pixel.offset * spread, easing: 'linear', fill: 'both' };
  ...
  const renderLayer = (content: React.ReactNode, index: number) => {
