...
export function InteractiveWorkspaceView({
  initialProjects,
}: InteractiveWorkspaceViewProps) {
  const [projects, setProjects] = useState<Project[]>(initialProjects)
  const [searchQuery, setSearchQuery] = useState('')
  const [filterCategory, setFilterCategory] = useState<'all' | 'active-tasks' | 'high-risk'>('all')
  ...
  const filteredProjects = projects.filter(p => {
    ...
    if (filterCategory === 'active-tasks') {
      return (p.tasks?.length ?? p.teamPulse?.tasksCompletedCount ?? 0) > 0
    }
    if (filterCategory === 'high-risk') {
      return (p.risks?.length ?? p.teamPulse?.risksCount ?? 0) > 0
    }
    return true
  })
        ...
        prev.map(p =>
          p.id === 'project-alpha'
            ? {
                ...
                teamPulse: {
                  ...p.teamPulse,
                  decisionsCount: (p.teamPulse?.decisionsCount || 0) + 1,
                },
              }
            : p
        )
  ...
  // Calculate aggregated stats with safe fallbacks
  const totalDecisions = projects.reduce((acc, p) => acc + (p.decisions?.length ?? p.teamPulse?.decisionsCount ?? 0), 0)
  const totalTasks = projects.reduce((acc, p) => acc + (p.tasks?.length ?? p.teamPulse?.tasksCompletedCount ?? 0), 0)
  const totalRisks = projects.reduce((acc, p) => acc + (p.risks?.length ?? p.teamPulse?.risksCount ?? 0), 0)
  const totalUnresolved = projects.reduce((acc, p) => acc + (p.unresolvedIssues?.length ?? p.teamPulse?.unresolvedCount ?? 0), 0)
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold hover:bg-emerald-500/20 hover:scale-105 active:scale-95 transition-all shadow-xs"
          >
            <Volume2 className="w-3.5 h-3.5" />
            <span>Enroll Voice ID</span>
          </button>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {filteredProjects.map((project) => {
            ...
            const decisionsCount = project.decisions?.length ?? project.teamPulse?.decisionsCount ?? 0
            const tasksCount = project.tasks?.length ?? project.teamPulse?.tasksCompletedCount ?? 0
            const risksCount = project.risks?.length ?? project.teamPulse?.risksCount ?? 0
            const unresolvedCount = project.unresolvedIssues?.length ?? project.teamPulse?.unresolvedCount ?? 0
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 p-3 bg-white/[0.03] rounded-xl border border-white/[0.06] mb-6 text-xs">
                    <div className="flex items-center gap-2 text-slate-300">
                      ...
                      <span className="font-bold text-white">{decisionsCount}</span>
